function [Sigma,Alpsup,w0,pos,history,obj,status] = mklsvm_LpNorm(K,yapp,C,p,option,verbose)
...
%%% Initialization
%% Sigma is the kernel combination weight vector
Sigmaold = ((size(K,3))^(-1/p))*ones(size(K,3),1);
Sigma = Sigmaold;
kerneloption.matrix = sumKbeta(K,Sigmaold);
...
while loop & (nloop < nloopmax)
    ...
    for i = 1:length(Sigma)
        Sigma(i) = ((Sigmaold(i))^2) * Alpsup' * K(pos,pos,i) * Alpsup;
        Sigma(i) = (Sigma(i))^(1/(p+1));
    end
