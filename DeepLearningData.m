...
load('O14AJ_20141103.mat');
load('O14AJ_20141103_response.mat');
name='O14AJ';

% only load 4 DBS leads from each side and remove the rest
...
prompt=['1. Select bipolar channel by choosing  a number between 1 and 3'];
ch=input(prompt);
prompt=['2. Select the Left/Right channel by typing Left/Right'];
side=input(prompt);
prompt=['3. Select the Button/Audio/Video/Reach/Random'];
target=input(prompt);

% ==================== End. Load Data =======================


% ==== Start. Calculate Bipolar Signal for each DBS lead=====
...
clear signal
% ===== End. Calculate Bipolar Signal for each DBS lead======


% ============ Start. Find Cue, onset & offset ==============
...
% ============= End. Find Cue, onset & offset ===============


% =======Start. Set 1sec before and 3secs after onset =======
% Make a window of the same length for all events, 
% centered around onset
...
% ========End. Set 1sec before and 2secs after onset ========


% ======Start. Compute Time/Frequency Map and PSD Graph======
...
for count=1:LoopNumber
    ...
    % Compute TimeFrequency components      
...
clear BipolarSignal TF TF1 TF2 TF3
...
% ======End. Compute Time Frequency Map using CWT/WELCH Method =======
