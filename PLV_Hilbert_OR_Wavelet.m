...
function [map]=PLV_Hilbert_OR_Wavelet(SubjectName,SubjectResponse,name,LFPChannel,ECoGChannel,SideLFP,SideECoG,Task,DownSamp)
...
StartPoint=-3*Fs;       % StartPoint=round(-3*Fs);
...
%        TempMap = abs(compute_plv_hilbert(TempSignalLFP,TempSignalECoG,fw,Fs,10,100,0,1));
       TempMap = abs(compute_plv_wavelet(TempSignalLFP,TempSignalECoG,fw,Fs,0,'MATLAB'));
       TempMap = TempMap';
%        size(TempMap)
       % ============================================== 
...
%            disp('Authomatically set Gap!!')
           disp('Baseline is out of range. You have to decrease the "StartPoint" or increase the "BaseStart" and "Gap"')
           return
