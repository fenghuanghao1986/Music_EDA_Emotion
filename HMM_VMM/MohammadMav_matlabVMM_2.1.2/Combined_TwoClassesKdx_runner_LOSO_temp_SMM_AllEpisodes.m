...
%             epis = 1; % FF = 1, SF = 2, RE = 3;
            ...
            jVmmTD_full = vmm_createNew(seqTD,  ALGS{s}, paramsTD);
             disp(sprintf('Pr(A | AAAB) = %f', vmm_getPr(jVmmTD_full, mapS(ab,'A'), mapS(ab,'A'))));
            ...
            VMM.T(T,1) = T;
            VMM.NumSymb(T,1) = 2.^T;
            VMM.epis = Episod_ID{epis};
            VMM.logLikASD = logLikASD;
            VMM.logLikTD = logLikTD;
            VMM.resultsASD=resultsASD;
            VMM.resultsTD=resultsTD;
            
    end
    PathData = 'C:\Documents and Settings\Vision\My Documents\Dave\Gaze Project\MohammadMavadati\MiamiProjectGaze\Saved';
    PathEpisode = [PathData '\Episode_'  Episod_ID{epis} '\VMM_Results'];
    
    VMM_Results=VMM;
    save(PathEpisode, 'VMM')
