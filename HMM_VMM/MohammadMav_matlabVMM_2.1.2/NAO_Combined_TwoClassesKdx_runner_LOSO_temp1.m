...
for i_Subj =1:NumSubj
    ...
    for i_seq = 1:NumSeqs
        ...
        subjASD_List_SameLength(itr) = ASD_Tot_Subj_list(i_Subj);
        itr = itr+1;
    end
end
...
for i_Subj =1:NumSubj
    ...
    for i_seq = 1:NumSeqs
        ...
        subjTD_List_SameLength(itr) = TD_Tot_Subj_list(i_Subj);
        itr = itr+1;
    end
end
    ...
    case 'oneSeqPerSubj'
        ...
        for (i_seq = 1:size(seqTD_OverallSubj,2) ) 
            if (~isempty(seqTD_OverallSubj{i_seq}))
                seqTD{itr} = seqTD_OverallSubj{i_seq};
                subjTD(itr) = subjTD_List(i_seq);
                itr=itr+1;
            end
        end
        seqASD = seqASD_OverallSubj;
        subjASD = subjASD_List;
