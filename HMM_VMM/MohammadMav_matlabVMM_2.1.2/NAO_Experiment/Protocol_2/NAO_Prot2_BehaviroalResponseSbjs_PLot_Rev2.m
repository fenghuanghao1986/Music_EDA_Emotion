...
close all
clc
% NAO_Prot2_BehaviroalResponseSbjs_PLot_Rev2: plot the reslts of each
% subject in separate figure, with date, task and accuracy response for
% each subtask 
...
Figure_Path = fullfile(File_Path,['BehaviralResponse' LastDateCoded]);
mkdir(Figure_Path);
...
for i_sub = 1:NumSubjs
        ...
        if (i_task ==1)%%extract the data of the experiment from the excel sheet
           ...
           %%check baseline vs Inervention by checking 5th column in the same row as IndRowDate 
            ...
            for i_sess=1:NumSessions
                ...
                if (isnan(ID_interv))
                    GameType =[GameType ; ['BaseLine' num2str(4-base_itr)]];
                    base_itr=base_itr+1;
                else 
    ...
    IndCodedSessions = find (sum(Task_Accuracy,2) ~= -NumTasks)% remove the TableTemplates from each spreadSheet of excel
    Subj_Task_Accuracy{i_sub}.IndCodedSessions = IndCodedSessions; %All the sessions, Tasks for each subject That has been coded
    ...
    h = figure(i_sub), 
    Ind = IndCodedSessions(end:-1:1)% reverse the indices to have baseline first then intervenstions
    bar(Task_Accuracy(Ind,:),'DisplayName','Task_Accuracy');
    set(gca,'XTickLabel',GameType(Ind,:))
    title(SelectedSubj , 'FontSize', 15),
    xlabel ('Session ID (BaseLine vs Intervention)','FontSize', 15); 
    ylabel ('Response Accuracy (%)','FontSize', 15)
    grid on 
    legend( 'T1', 'T2.1', 'T2.2' ,'T3' ,'T4.1', 'T4.2', 'T5', 'Location', 'NorthWestOutside')
    saveas(h, [Figure_Path '\' SelectedSubj '.png'],'png');
    saveas(h, [Figure_Path '\' SelectedSubj '.fig'],'fig');
end
